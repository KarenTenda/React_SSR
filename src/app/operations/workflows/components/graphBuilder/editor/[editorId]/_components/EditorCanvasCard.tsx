...
import { EditorCanvasCardType } from '@/app/operations/workflows/types/EditorCanvasTypes';
import { DataTypesColors } from '@/app/operations/workflows/types/PinMetadataTypes';
...
const DefaultEditorCanvasCard = ({ data }: { data: EditorCanvasCardType }) => {
  ...
  useEffect(() => {
    ...
    console.log('Current Card Node:', currentNode);
    ...
    <>
      ...
      <Card
        onClick={(e) => {
          ...
          console.log('Selected card Element:', val?.data.metadata);
          if (val && state.editor.selectedNode?.id === '')
            dispatch({
            ...
            dispatch({
              type: 'SELECTED_ELEMENT',
              payload: {
                element: state.editor.selectedNode,
              },
            });
        }}
